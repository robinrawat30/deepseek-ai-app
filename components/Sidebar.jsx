            ...
            <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
                <div>
                    <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
                        <Image src={expand ? assets.logo_text : assets.logo_icon} alt='logo' className={`${expand ? "w-35" : "w-10"}`} />
                        ...
                        <div onClick={() => expand ? setExpand(false) : setExpand(true)} className=" group relative flex items-center  justify-center hover:bg-gray-500/20 transition-all duration-300 h-9 w-9 aspect-square rounded-lg cursor-pointer">
                            {/* <Image src={assets.menu_icon} alt='' className={`expand ? assests.logo_text : assest.logo_icon`} /> */}
                            <Image src={expand ? assets.sidebar_close_icon : assets.sidebar_icon} alt='' className="hidden md:block w-7" />

                            <div className={` absolute w-max ${expand ? 'left-1/2 -translate-1.5 top-12' : '-top-12 left-0'} opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none`}>
                    ...
                    <button className={`mt-8 flex items-center justify-center cursor-pointer ${expand ? " bg-blue-500 hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max " : "group relative h-9 w-9 m-auto hover:bg-gray-500/30 rounded-lg"}`}>
                        <Image src={expand ? assets.chat_icon : assets.chat_icon_dull} alt='' className={`${expand ? assets.chat_icon : assets.chat_icon_dull}`} />
                        ...
                        {
                            expand && <p className=" text-white text">New Chat</p>
                        }
                    </button>
                ...
                <div>
                    <div className={`flex items-center cursor-pointer group relative ${expand ? " gap-1 text-white/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/10 cursor-pointer " : "h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
                        ...
                        <div className={` absolute -top-60 pb-8 ${!expand && "-right-40"} opacity-0 group-hover:opacity-100 hidden group-hover:block transition`}>
                            ...
                            <div className="relate w-max bg-black text-white text-sm p-3 rounded-lg shadow-lg">
                            </div>
